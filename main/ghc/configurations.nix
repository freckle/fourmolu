...
let
  ...
  ghcOverlay = ghcVersion: override: final: prev:
    prev // {
      haskell = prev.haskell // {
        compiler = prev.haskell.compiler // {
          ${ghcVersion} =
            prev.haskell.compiler.${ghcVersion}.overrideAttrs override;
        };
      };
    };
in {
  ghc-9-2-7 = { packageSelection, enableHLS }:
    let
      nixpkgs = import inputs.nixpkgs-stable-2023-07-25 {
        ...
        overlays = [
          (ghcOverlay "ghc927"
            (addPatches [ ./sanity-check-find-file-name.patch ]))
        ];
      };
      name = "ghc927";
      ...
      hls = nixpkgs.haskell-language-server.override {
    ...
    in symlinkJoin {
      inherit name;
      paths = [ ghcWithPackages weeder cabal stack ]
